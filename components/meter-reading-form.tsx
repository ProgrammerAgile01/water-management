...
export function MeterReadingForm() {
  ...
  const [periodOptions, setPeriodOptions] = useState<PeriodOpt[]>([]);
  const [periodLoading, setPeriodLoading] = useState(false);

  // Awalnya kosong; akan diisi dari query/setting/periode
  ...
  useEffect(() => {
    ...
    const loadPeriods = async () => {
      ...
      try {
        const res = await fetch("/api/catat-periode/options", {
          cache: "no-store",
        });
        const data = await safeJson(res);

        if (!cancelled && data?.ok && Array.isArray(data.options)) {
          setPeriodOptions(
  ...
  const tagihanText = useMemo(() => {
    if (!selectedPeriod) return "";
    const [y, m] = selectedPeriod.split("-").map(Number);
    const tagih = new Date(y, m, 1);
    return `Untuk Penagihan ${tagih.toLocaleDateString("id-ID", {
      month: "long",
      year: "numeric",
    })}`;
  }, [selectedPeriod]);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 items-end">
        ...
        <div className="space-y-2">
          ...
          <Select
            ...
            disabled={isFinalizingPeriod || isLoading || periodLoading}
          >
            <SelectTrigger className="h-12 bg-card/50">
              <SelectValue
                placeholder={
                  periodLoading ? "Memuat periode..." : "Pilih periode..."
                }
              />
            </SelectTrigger>
