...
function DraggableRow({
  ...
  return (
    <tr ref={setNodeRef} style={style} {...attributes} className="border-b border-border/10 hover:bg-muted/20">
      {/* drag handle */}
...
export function CustomerList() {
  ...
  const changePageSize = (val: number) => {
    setPageSizeUI(val)
    setCurrentPage(1)
    try { localStorage.setItem("tb.pelanggan.pageSize", String(val)) } catch {}
  }
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      if (!res.ok || !json?.ok) throw new Error(json?.message || "Gagal menghapus pelanggan")
      toast({ title: "Pelanggan Dihapus", description: `Data ${customerToDelete.nama} berhasil dihapus.` })
  ...
  const onDragEnd = async (event: DragEndEvent) => {
    ...
    await mutate(
      ...
      (prev?: ApiResp) => {
        ...
        moved.forEach((row, idx) => {
          ...
          if (idxInItems >= 0) items[idxInItems] = { ...items[idxInItems], noUrutRumah: idx + 1 }
        })
        return { ...prev, items }
      },
      { revalidate: false }
    ...
    try {
      ...
      const json = await res.json()
      if (!res.ok || !json?.ok) throw new Error(json?.message || "Gagal mengubah urutan")
  ...
  return (
    <>
      <GlassCard className="p-6">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-6">
          ...
          <div className="flex w-full lg:w-auto items-stretch gap-3">
            ...
            />
            {/* Page size dropdown — gaya disamakan dengan input */}
            <div className="flex items-center gap-2">
              <span className="text-sm text-muted-foreground hidden sm:block">Tampilkan</span>
              <select
                value={pageSizeUI}
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-border/20">
                        ...
                        <th className="text-center py-3 px-2 text-sm font-medium text-muted-foreground">Zona</th>
                        <th className="text-center py-3 px-2 text-sm font-medium text-muted-foreground">No</th>
                    ...
                    <tbody>
                      {rows.map((customer) => (
                        <DraggableRow key={customer.id} customer={customer}>
                          ...
                          <td className="py-3 px-2 text-sm text-center text-foreground">{customer.zonaNama ?? "-"}</td>
                          <td className="py-3 px-2 text-sm text-center text-foreground">{customer.noUrutRumah ?? "-"}</td>
                      ...
                      {rows.length === 0 && (
                        <tr>
                          <td className="py-6 px-2 text-center text-sm text-muted-foreground" colSpan={10}>
                            Tidak ada data.
                          </td>
                        </tr>
                      )}
              ...
              {rows.map((customer) => (
                <div key={customer.id} className="p-4 bg-muted/20 rounded-lg space-y-3">
                  ...
                  <div className="space-y-1">
                    <p className="text-sm text-muted-foreground"><span className="font-medium">WA:</span> {customer.noWA}</p>
                    <p className="text-sm text-muted-foreground"><span className="font-medium">Alamat:</span> {customer.alamat}</p>
                    <p className="text-sm text-muted-foreground"><span className="font-medium">Zona:</span> {customer.zonaNama ?? "-"}</p>
                    <p className="text-sm text-muted-foreground"><span className="font-medium">No:</span> {customer.noUrutRumah ?? "-"}</p>
                    <p className="text-sm text-muted-foreground"><span className="font-medium">Meter Awal:</span> {customer.meterAwal}</p>
                  </div>

                  <div className="flex gap-2 pt-2">
                    <Button size="sm" variant="outline" onClick={() => handleViewHistory(customer)} className="flex-1 bg-transparent">
                      <Eye className="w-4 h-4 mr-2" />Histori
                    </Button>
            ...
            {total > 0 && (
              <div className="flex items-center justify-between mt-6">
                <p className="text-sm text-muted-foreground">
                  Menampilkan {(page - 1) * pageSize + 1}–{Math.min(page * pageSize, total)} dari {total} pelanggan
                </p>
                <div className="flex gap-2">
                  <Button variant="outline" size="sm" onClick={() => setCurrentPage((p) => Math.max(p - 1, 1))} disabled={page <= 1} className="bg-transparent">
                    Sebelumnya
                  </Button>
                  <Button variant="outline" size="sm" onClick={() => setCurrentPage((p) => Math.min(p + 1, totalPages))} disabled={page >= totalPages} className="bg-transparent">
                    Selanjutnya
                  </Button>
                </div>
              </div>
      ...
      {showHistory && selectedCustomer && (
        <CustomerHistoryModal customer={selectedCustomer} onClose={() => { setShowHistory(false); setSelectedCustomer(null) }} />
      )}
