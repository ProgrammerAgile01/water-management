...
//   // hanya periode berjalan yg bisa dilihat
...
export function BillingTable() {
  ...
  const canInput = (b: BillingItem) => (b.canInputPayment ?? (b.status !== "lunas" && currentPeriode && b.periode === currentPeriode));
  ...
  const refreshData = async () => {
    setIsLoading(true);
    try {
      const headers: HeadersInit = authUser ? { "x-user-id": authUser.id, "x-user-role": authUser.role } : {};
      ...
      const url = `/api/tagihan${qs.toString() ? `?${qs.toString()}` : ""}`;
      ...
      // periode dari API (role-aware)
      const cps: string[] = json?.meta?.periodes ?? [];
      const cp: string = json?.meta?.currentPeriode ?? "";
      setCurrentPeriode(cp);

      if (authUser && (authUser.role === "ADMIN" || authUser.role === "PETUGAS")) {
        setPeriodeOptions([{ value: "semua", label: "Semua Periode" }, ...cps.map((p) => ({ value: p, label: p }))]);
      } else {
        // WARGA: hanya periode berjalan (API sudah force)
        setPeriodeOptions(cps.map((p) => ({ value: p, label: p })));
        setSelectedPeriode(cp);
      }
    } catch (e) {
      console.error("fetch tagihan error:", e);
      setItems([]);
  ...
  // rows yang ditampilkan = hasil server (sudah role-filtered & paginated)
  ...
  // ====== UI COMMON: FILTERS ======
  function FiltersBar() {
    const isWarga = authUser?.role === "WARGA";
      ...
      <GlassCard className="p-4">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div className="space-y-2">
            ...
            {isWarga ? (
              <div className="py-2 px-3 rounded-md border border-border/50 bg-transparent">
                <span className="text-sm">{currentPeriode || "-"}</span>
              </div>
            ) : (
              <Select value={selectedPeriode} onValueChange={setSelectedPeriode}>
                <SelectTrigger>
          ...
          <GlassCard key={b.id} className="p-4">
            ...
            <div className="flex gap-2">
              ...
              <Button
                ...
                className="flex-1"
                disabled={!canInput(b)}
                title={!canInput(b) ? "Pembayaran hanya untuk periode berjalan" : undefined}
              >
                <CreditCard className="h-4 w-4 mr-2" /> Input Pembayaran
              </Button>
            </div>
