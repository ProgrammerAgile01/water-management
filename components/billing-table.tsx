...
import { Eye, FileText, CreditCard, Search, ChevronLeft, ChevronRight } from "lucide-react";
...
function formatPeriode(p?: string | null): string {
  ...
  const [mon, year] = norm.split(" ");
  if (mon && year && MONTH_INDEX[mon.toLowerCase()]) {
    return `${ID_MONTHS[MONTH_INDEX[mon.toLowerCase()] - 1]} ${year}`;
  }
...
export function BillingTable() {
  ...
  const [periodeOptions, setPeriodeOptions] = useState<Option[]>([{ value: "semua", label: "Semua Periode" }]);
  ...
  // periode terakhir dari API
  const [latestPeriode, setLatestPeriode] = useState("");

  // helpers
  const toApiStatus = (s: "semua" | "lunas" | "belum-lunas"): string | undefined => {
    if (s === "lunas") return "lunas";
    if (s === "belum-lunas") return "belum-lunas";
    return undefined;
  };
  ...
  const canInput = (b: BillingItem) =>
    (b.canInputPayment ?? (latestPeriode && b.periode === latestPeriode && b.status !== "lunas"));
  ...
  const refreshData = async () => {
    ...
    try {
      const headers: HeadersInit =
        authUser ? { "x-user-id": authUser.id, "x-user-role": authUser.role } : {};

      const qs = new URLSearchParams();
      if (selectedPeriode && selectedPeriode !== "semua") qs.set("periode", selectedPeriode);
      ...
      const res = await fetch(`/api/tagihan${qs.toString() ? `?${qs.toString()}` : ""}`, {
        cache: "no-store",
        headers,
      });
      ...
      setPeriodeOptions([{ value: "semua", label: "Semua Periode" }, ...periodes.map((p) => ({ value: p, label: p }))]);
    } catch (e) {
    ...
    return (
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 p-4">
        <div className="text-sm text-muted-foreground">
          Menampilkan <span className="font-medium">{rows.length ? from : 0}</span>–<span className="font-medium">{to}</span> dari{" "}
    ...
    return (
      <GlassCard className="p-4">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          {/* Search */}
          <div className="space-y-2">
            <label className="text-sm font-medium text-foreground">Cari Warga/Kode/Zona</label>
          ...
          <div className="space-y-2">
            ...
            <Button onClick={refreshData} className="w-full bg-transparent" variant="outline">
              Refresh Data
            </Button>
          </div>
        </div>
        ...
        {rows.map((b) => (
          <GlassCard key={b.id} className="p-4">
            <div className="flex items-start justify-between mb-3">
              <div>
                <h3 className="font-semibold text-foreground">Periode: {formatPeriode(b.periode)}</h3>
            ...
            <div className="space-y-3 mb-4">
              <div className="grid grid-cols-3 gap-2 text-sm">
                <div><span className="text-muted-foreground">Meter Awal:</span><p className="font-medium">{b.meterAwal ?? "-"}</p></div>
                <div><span className="text-muted-foreground">Meter Akhir:</span><p className="font-medium">{b.meterAkhir ?? "-"}</p></div>
                <div><span className="text-muted-foreground">Pemakaian:</span><p className="font-medium">{b.pemakaian ?? "-"} {b.pemakaian != null ? "m³" : ""}</p></div>
              </div>

              <div className="space-y-1 text-sm">
                <p className="text-muted-foreground">Rincian Tagihan:</p>
                <div className="pl-2 space-y-1">
                  <p>• {b.pemakaian ?? 0} x {fmtRp(b.tarifPerM3)} = {fmtRp(b.tarifPerM3 * (b.pemakaian ?? 0))}</p>
            ...
            <div className="flex gap-2">
              ...
              <Button
                ...
                className="flex-1"
                disabled={!canInput(b)}
                title={!canInput(b) ? "Pembayaran hanya untuk periode terakhir" : undefined}
              >
            ...
            {selected && (
              <div className="space-y-4">
                ...
                <div className="text-sm space-y-2">
                  ...
                  <p><span className="font-medium">Periode:</span> {formatPeriode(selected.periode)}</p>
                  <p><span className="font-medium">Total:</span> {fmtRp(selected.totalTagihan)}</p>
                  {selected.tanggalBayar && (
                    <p><span className="font-medium">Tanggal Bayar:</span> {new Date(selected.tanggalBayar).toLocaleString("id-ID")}</p>
                  )}
                </div>

                {selected.buktiPembayaran && (
                  <div className="text-center">
                    <img src={selected.buktiPembayaran || "/placeholder.svg"} alt="Bukti Pembayaran" className="max-w-full h-auto rounded-lg border" />
                  </div>
                )}

                <div className="flex gap-2">
                  <Button variant="outline" onClick={() => setSelected(null)} className="flex-1">Tutup</Button>
                </div>
              </div>
            )}
          </DialogContent>
                ...
                <tr key={b.id} className="border-b border-border/10 hover:bg-white/5">
                  <td className="p-4">
                    <div className="space-y-1">
                      <p className="font-medium text-foreground">Periode: {formatPeriode(b.periode)}</p>
                      <p className="text-sm text-muted-foreground">Nama Warga: {b.namaWarga}</p>
                      <p className="text-sm text-muted-foreground">Zona/Blok: {b.zona}</p>
                  ...
                  <td className="p-4">
                    <div className="space-y-1 text-sm">
                      ...
                      <p>Pemakaian: <span className="font-medium">{b.pemakaian ?? "-"} {b.pemakaian != null ? "m³" : ""}</span></p>
                    </div>
                  </td>
