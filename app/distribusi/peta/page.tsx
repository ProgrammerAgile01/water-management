...
type Resp = {
  ...
  periods: string[]; // ["2025-09", ...] ordered desc
...
function fmtPeriodeID(kode?: string | null) {
  if (!kode) return "-";
  const [y, m] = kode.split("-").map(Number);
  const name = MONTHS_ID[(m ?? 1) - 1] || "";
  return `${name} ${y}`;
}

export default function DistribusiPetaTablePage() {
  // ===== Filter state =====
  const [periode, setPeriode] = useState<string>(""); // value tetap "YYYY-MM"
  const [zonaId, setZonaId] = useState<string | "ALL">("ALL");
  const [threshold, setThreshold] = useState<string>("0.5");
  const [baselineN, setBaselineN] = useState<string>("6"); // 3/6/12
  const [query, setQuery] = useState<string>("");
  const [tab, setTab] = useState<"ALL" | "NORMAL" | "ANOMALY" | "ZERO">("ALL");

  // Build querystring untuk API
  const qs = useMemo(() => {
    const p = new URLSearchParams();
    if (periode) p.set("periode", periode);
    if (zonaId && zonaId !== "ALL") p.set("zonaId", zonaId);
  ...
  // Ambil default periode dari server saat pertama kali
  useEffect(() => {
  ...
  const stat = useMemo(() => {
    ...
    return {
      ...
      lowHistory: all.filter((r) => r.baselineCount <= 1).length, // untuk banner
    };
  }, [data]);
  ...
  // Export Excel (pakai data yang sudah difilter)
  const exportExcel = () => {
  ...
  return (
    <AppShell>
      ...
      <div className="space-y-4">
        {/* ===== Toolbar: tombol aksi agar selalu terlihat ===== */}
        {/* <div className="flex items-center justify-end gap-2">
          <div className="text-sm text-muted-foreground">
            Periode Aktif:{" "}
            <span className="font-semibold">{fmtPeriodeID(data?.periode)}</span>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
            ...
            <div className="flex flex-col">
              ...
              >
                <SelectTrigger>
                  <SelectValue
                    placeholder={fmtPeriodeID(data?.periode) || "Pilih periode"}
                  />
                </SelectTrigger>
                <SelectContent>
                  {(data?.periods || []).map((p) => (
            ...
            <div className="flex flex-col">
              ...
              >
                ...
                <SelectContent>
                  ...
                  <SelectItem value="0.7">Longgar (±70%)</SelectItem>
                </SelectContent>
              </Select>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
          <GlassCard className="p-4 flex items-center gap-3">
            ...
            <div>
              <div className="text-xs text-muted-foreground">Periode Dipilih</div>
        ...
        >
          <TabsList className="grid grid-cols-4 w-full md:w-auto">
            ...
            <TabsTrigger value="ANOMALY">Anomali</TabsTrigger>
        ...
        <div className="grid gap-3 md:hidden">
          ...
          {pageRows.map((r, idx) => (
            <GlassCard key={r.pelangganId} className="p-4">
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="text-right font-bold">Pemakaian (m³)</TableHead>
