...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    for (const t of tagihanTahunIni) {
      ...
      // 2) kalau tidak ada, fallback ke P-1
      if (!usageM3) {
        const prev = prevOfPeriode(y, m);
      ...
      yearlyUsage.push({
        month: monthShort(mm),
        usage: usageM3, // sekarang akan terisi 9 m³, dst jika datanya ada
        bill: t.totalTagihan,
        status: ["paid", "lunas"].includes((t.statusBayar || "").toLowerCase())
          ? "paid"
          : "unpaid",
      } as { month: string; usage: number; bill: number; status: "paid" | "unpaid" | "pending" });
    }
